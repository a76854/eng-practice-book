...
本实验对应理论 [第9章 与外部世界的集成](../../part4_现代工程进阶与交付/chapter09_与外部世界的集成/index.md)。建议先通读第9章 9.1 至 9.3 节的第三方集成模式、语音识别接入与大模型流式接口，再阅读 `m2t/asr.py` 的 `normalize_result` 与 `m2t/llm.py` 的 `LLMClient` / `map_llm_error`，最后参考 `book/part4_现代工程进阶与交付/chapter09_与外部世界的集成/demo_llm_stream.py` 的 SSE 仿真思路，再动手。你会在本实验中用 FastAPI 打通录音上传、实时转写与 AI 总结的完整链路，用 EventSource 在前端做流式渲染，并在本机用 mock 完成可演示的联调闭环。
...
1. 阅读 [第9章 9.1 第三方服务集成模式](../../part4_现代工程进阶与交付/chapter09_与外部世界的集成/9.1_第三方服务集成模式.md) 中关于超时、重试与错误脱敏的讨论，理解为何外部调用的配置与错误映射要收敛在一处。
2. 阅读 [第9章 9.2 语音识别接入](../../part4_现代工程进阶与交付/chapter09_与外部世界的集成/9.2_语音识别接入.md) 与 [9.3 大模型接口设计](../../part4_现代工程进阶与交付/chapter09_与外部世界的集成/9.3_大模型接口设计.md)，重点关注音频格式归一、结果多形状归一、结构化输出与 SSE 增量解析。
...
1. 进入 `labs/lab07_前后端联调与流式/starter`，阅读 `README.md`、`main.py`、`index.html` 与 `requirements.txt`，梳理“后端 SSE 接口、前端 EventSource 订阅、mock 数据”三层的依赖方向。
