...
本实验对应理论 [第1章 开发者的元技能](../../part1_软件工程筑基/chapter01_开发者的元技能/index.md) 与 [第9章 与外部世界的集成](../../part4_现代工程进阶与交付/chapter09_与外部世界的集成/index.md)。建议先通读第1章 1.4 节的自动化脚本与第9章 9.1 至 9.2 节的第三方服务集成与音频归一，再动手。你会在本实验中以纯命令行形态完成 MeetingToText 的首个可交付切片，体会不依赖 Web 框架时如何把业务逻辑与输入输出干净分开。
...
1. 阅读 [第1章 1.4 Python 自动化脚本](../../part1_软件工程筑基/chapter01_开发者的元技能/1.4_Python自动化脚本.md) 中关于 `argparse`、`subprocess` 与脚本封装的讨论，理解为何超过 10 行的自动化一律用 Python 而非 Shell。
2. 阅读 [第9章 9.1 第三方服务集成模式](../../part4_现代工程进阶与交付/chapter09_与外部世界的集成/9.1_第三方服务集成模式.md) 与 [9.2 语音识别接入](../../part4_现代工程进阶与交付/chapter09_与外部世界的集成/9.2_语音识别接入.md)，留意音频格式归一、结果多形状归一与错误脱敏的思想。
...
1. 打开 `labs/lab03_里程碑A_CLI转写/starter/main.py`，运行 `python main.py --help` 与 `python main.py transcribe --help`，观察子命令、选项与帮助文本的组织方式。
