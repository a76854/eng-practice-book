...
本实验对应理论 [第4章 HTTP 与 RESTful 架构](../../part2_后端开发全景与核心基石/chapter04_HTTP与RESTful架构/index.md) 与 [第5章 数据持久化：从 SQL 到 ORM](../../part2_后端开发全景与核心基石/chapter05_数据持久化从SQL到ORM/index.md)。建议先通读第4章 4.1 至 4.4 节的 HTTP 语义与 FastAPI 路由，再通读第5章 5.3 至 5.4 节的数据库访问三境界与迁移思想，再动手。你会在本实验中用 FastAPI 实现会议记录的增删改查接口，并用版本化思想管理表结构演进。
...
1. 阅读 [第4章 4.1 HTTP 协议精髓](../../part2_后端开发全景与核心基石/chapter04_HTTP与RESTful架构/4.1_HTTP协议精髓.md) 至 [4.4 异常处理与全局响应](../../part2_后端开发全景与核心基石/chapter04_HTTP与RESTful架构/4.4_异常处理与全局响应.md)，留意方法语义、状态码与统一响应的协作价值。
2. 阅读 [第5章 5.3 Python 操作数据库三境界](../../part2_后端开发全景与核心基石/chapter05_数据持久化从SQL到ORM/5.3_Python操作数据库三境界.md) 与 [5.4 数据库迁移 Alembic](../../part2_后端开发全景与核心基石/chapter05_数据持久化从SQL到ORM/5.4_数据库迁移Alembic.md)，理解参数化查询与版本化迁移的必要性。
...
1. 打开 `labs/lab04_RESTful_API与数据库/starter/main.py`，运行 `python -c "import ast; ast.parse(open('starter/main.py').read()); print('parse ok')"` 确认文件可解析。
