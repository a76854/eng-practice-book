...
本实验对应理论 [第2章 构筑代码质量的护城河](../../part1_软件工程筑基/chapter02_构筑代码质量的护城河/index.md)。建议先通读该章的 2.1 至 2.5 节，再动手。你会在本实验中为给定的工具函数补齐 pytest 用例，配置 mypy 与 Ruff 至严苛级别，并体会 CI 质量门禁如何拦截低质量合并。
...
1. 阅读 [第2章 2.1 类型系统与防御性编程](../../part1_软件工程筑基/chapter02_构筑代码质量的护城河/2.1_类型系统与防御性编程.md) 至 [2.5 测试覆盖率与质量门禁](../../part1_软件工程筑基/chapter02_构筑代码质量的护城河/2.5_测试覆盖率与质量门禁.md)，留意 `mypy` 严格选项与 `Ruff` 规则含义。
2. 进入 `labs/lab02_单元测试与静态检查/starter`，按其 `README.md` 安装最小依赖（若使用 `.venv`，先 `python -m venv .venv` 并激活）。
