...
本实验对应理论 [第11章 部署、容器化与持续集成](../../advanced_engineering/deploy_cicd/index.md)。建议先通读第11章 11.1 至 11.4 节的部署演进、Dockerfile 最佳实践、Compose 编排与 CI 流水线，再对照 `labs/lab08_fullstack_container/starter/` 的 Dockerfile 与 Compose 起手骨架，最后动手。你会在本实验中把全栈应用容器化，用 Compose 在本地完成联调预演，并整理答辩材料，完成从可运行到可交付的收口。
...
1. 阅读 [第11章 11.1 部署演进史](../../advanced_engineering/deploy_cicd/deployment_evolution.md) 至 [11.2 Dockerfile 最佳实践](../../advanced_engineering/deploy_cicd/dockerfile_best_practices.md)，理解镜像分层、层缓存键与选择性 COPY 的权衡。
2. 阅读 [第11章 11.3 Docker Compose 编排](../../advanced_engineering/deploy_cicd/docker_compose_orchestration.md) 与 [11.4 CI/CD 流水线](../../advanced_engineering/deploy_cicd/cicd_pipeline.md)，重点关注声明式 YAML 如何表达服务拓扑，以及 GitHub Actions 的工作流、作业与步骤模型。
...
3. 对照 [11.4 节的内联工作流示例](../../advanced_engineering/deploy_cicd/cicd_pipeline.md) 的校验与测试链路，明确 CI 如何把 ruff、mypy、pytest 与 `docker compose config -q` 串为门禁。
