...
> 如果把动态类型比作"口头约定"，类型标注就是"书面合同"——口头约定灵活但易生歧义，就像你在电话里和朋友说"帮我带个东西"，对方可能带回来一瓶水、一本书或一包纸巾；书面合同让双方在动手前就对齐了形状、尺寸与交付时间。

类型标注的实质，是把动态类型中“运行到才知道对错”的隐含风险，前置为“写出来就能检查”的明确边界。它不改变Python的运行时行为，但它改变了你与代码之间的信任关系，从“我相信它是对的”变成“我能验证它是对的”。动态类型像一条没有围栏的栈道，走得快，但每一步都需要自己判断脚下是否踏实；类型标注像在栈道两侧加了扶手，它不是让你走得更慢，而是让你在加快步伐时不必频繁低头确认踩的是不是空的。这层扶手，在单人徒步时似乎多余，但队伍越庞大、步速越快，它的价值就越明显——不是为了限制你，而是为了让你走得更稳、更远。
...
- 最贵的 bug 往往不一定在于算法，往往在于基础的数据结构，传错类型、用错空值、拼错字段。
- 类型标注、dataclass、pydantic 逐层递进，分别管理数据类型、内部结构、运行时边界。
- 坏数据要在入口拦住，不让它走到深层代码里，里面只处理合法数据，这就是尽早失败。
- 编辑器写的时候提醒，mypy 提交前再查一遍，pydantic 在运行时守边界。