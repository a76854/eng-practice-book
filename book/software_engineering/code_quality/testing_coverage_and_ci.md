...
- 手动测试接口，短时低效且不完全，而且无法复现，改用自动化测试才能保证代码质量稳定，即使代码有大的变化，也有自动化测试门禁兜底，防止误提交。
- 单元测试测逻辑，集成测试测协作，端到端测试测整条链，数量依次递减，抽象层级依次递增。
- 测试设置前置条件复用用 `fixture`，一个例子多个参数用 `parametrize`，隔离外部依赖用 `mock`。
- 测试覆盖率 100% 不代表没有bug，覆盖关键路径更重要。