...
MeetingToText 的后台（任务列表、搜索、播放、纪要编辑）属于**交互中等、以后端 API 为核心**的系统，选型上更看重“后端开发者能快速读懂前端目录与契约、联调成本低”，这也是后续 [7.3 为何选择 Vue 3 + Vite](7.3_为何选择Vue3_Vite.md) 的判断起点之一，但不意味着在其他约束下 Vue 一定最优——约束变了，答案就该变。

## 可运行示例：用 Python 类比三种“状态 → 视图”链路

文件 `book/part3_前端协作与现代前端基础/chapter07_前端开发概况与工程化演进/demo_frameworks_compare.py`（用纯 Python 类比三种框架的更新模型：React 的不可变快照、Vue 的代理自动追踪、Angular 的服务注入，本地可复现，无网络）：

```{code-cell} ipython3
# 文件 book/part3_前端协作与现代前端基础/chapter07_前端开发概况与工程化演进/demo_frameworks_compare.py
...
> **工程启示**：框架是“约束的集合”，而非“能力的排名”。React 用显式换可预测，Vue 用代理换直觉，Angular 用约定换一致。读懂它们的约束，你就能在给定团队与项目约束下，给出可解释、可复盘的选型判断——这正是后端视角参与前端协作的关键能力。后续 [7.4 前端工程化基石](7.4_前端工程化基石.md) 将把“框架之外的工程三件套”补齐。
