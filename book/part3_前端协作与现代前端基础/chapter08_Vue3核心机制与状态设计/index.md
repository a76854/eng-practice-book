...
- [8.1 响应式原理](8.1_响应式原理.md) —— Proxy vs defineProperty：依赖追踪如何工作；组合式 API 生命周期时机
- [8.2 组件化设计](8.2_组件化设计.md) —— Props / Emits / Slots 三条通信链路；组合函数复用边界
- [8.3 路由管理](8.3_路由管理.md) —— Vue Router 4 历史模式、路由守卫与懒加载
- [8.4 跨组件状态管理 Pinia](8.4_跨组件状态管理Pinia.md) —— Store / Action / Getters 模块化；与后端状态的边界
...
> **跨平台约定**：本章所有涉及路径与环境激活的命令均标注 Windows / macOS / Linux 差异，详见各小节对照表；正文跨章引用一律使用相对链接，如 [第1章 开发者的元技能](../../part1_软件工程筑基/chapter01_开发者的元技能/index.md) 与 [第7章 前端概况](../chapter07_前端开发概况与工程化演进/index.md)。

文件 `book/part3_前端协作与现代前端基础/chapter08_Vue3核心机制与状态设计/demo_index.py`（验证本章环境与 Vue 3 核心概念的最小协作闭环，本地可复现）：

```{code-cell} ipython3
# 文件 book/part3_前端协作与现代前端基础/chapter08_Vue3核心机制与状态设计/demo_index.py
