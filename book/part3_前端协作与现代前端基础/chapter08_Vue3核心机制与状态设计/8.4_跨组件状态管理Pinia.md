...
文件 `book/part3_前端协作与现代前端基础/chapter08_Vue3核心机制与状态设计/demo_pinia.py`（用 Python 模拟 Pinia 的三层模型与前后端状态边界，本地可复现，无网络）：

```{code-cell} ipython3
# 文件 book/part3_前端协作与现代前端基础/chapter08_Vue3核心机制与状态设计/demo_pinia.py
...
> **与全书的衔接**：本节的 Pinia 单例与 [第5章 数据持久化](../../part2_后端开发全景与核心基石/chapter05_数据持久化从SQL到ORM/index.md) 的后端 `TaskStore` 形成镜像——前端管“交互缓存”，后端管“持久真相”；两者的同步点正是 [第4章的 HTTP 契约](../../part2_后端开发全景与核心基石/chapter04_HTTP与RESTful架构/index.md)（`GET /api/tasks`）与 [实验 06 的前后端联调](../../part5_实验指导书/experiment06_前端页面与路由状态管理/index.md)。
