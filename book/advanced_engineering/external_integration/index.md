...
- [第三方服务集成模式](third_party_service_integration.md) —— SDK vs HTTP 客户端：何时包一层、何时直接调；超时、重试、熔断与错误脱敏
- [语音识别接入](asr_integration.md) —— 云端签名的本质、音频格式归一（声道/采样率/编码）与多形状结果归一
- [大模型接口设计](llm_api_design.md) —— 结构化输出（JSON 模式）、流式 SSE 的增量解析与首字时延
- [异步任务队列 Celery](async_task_queue_celery.md) —— 为何长时任务要异步化；broker / worker / result 的最小队列抽象（不依赖 Celery）
...
> **环境约定**：本书面向 Linux，本章命令均面向 Linux，路径与环境激活统一使用 `source .venv/bin/activate` 与 `/` 分隔符；正文跨章引用一律使用相对链接，如 [第1章 开发者的元技能](../../software_engineering/dev_meta_skills/index.md) 与 [第6章 并发模型与性能工程](../../backend_development/concurrency_perf/index.md)。
