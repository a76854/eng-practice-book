...
### 用 `fetch` 消费 API：`api/client.ts` 的封装
...
#### 实验：把 `ref(0)` 改为 `reactive({ count: 0 })` 并去掉 `.value` → 预测模板与脚本行为
...
#### 实验：把 `v-model="keyword"` 改为 `:value="keyword"`（删掉 `@input`）→ 预测输入行为
...
#### 实验：把 `fetch('/api/tasks')` 改为 `fetch('/api/tasks?status=done')` 并去掉前端 `filter` → 预测数据与职责
...
#### 实验：把 `onMounted(load)` 改为按钮点击才 `load` 且删掉 `loading` 状态 → 预测首屏与并发点击
