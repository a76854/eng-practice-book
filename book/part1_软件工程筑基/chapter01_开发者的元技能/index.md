...
- [1.1 工程化项目结构](1.1_工程化项目结构.md) —— 为什么 `src` 布局能避免导入陷阱，`pyproject.toml` 如何统一 PEP 517/518/621 的构建与元数据
- [1.2 依赖与虚拟环境](1.2_依赖与虚拟环境.md) —— `venv` / `conda` / `uv` 的选型与隔离原理
- [1.3 Shell、文件系统、进程与管道](1.3_Shell文件系统进程管道.md) —— 把 Shell 当作“可组合的文本流水线”来理解
- [1.4 Python 自动化脚本](1.4_Python自动化脚本.md) —— 用 Python 代替人肉重复劳动
- [1.5 Git 核心模型与工作流](1.5_Git核心模型与工作流.md) —— 从对象模型看懂分支，再选对工作流

此外，本章所有示例均可在书仓根目录的 `.venv` 环境中复现；涉及 MeetingToText 的片段统一通过教学包 `m2t` 复用（见 [m2t 源码](../../..//m2t/audio.py) 的精简实现），无需启动真实的 ASR 或 LLM 服务。

> **跨平台约定**：本章所有涉及路径与环境激活的命令均标注 Windows / macOS / Linux 差异，详见各小节对照表；正文跨章引用一律使用相对链接，如 [第2章 构筑代码质量的护城河](../chapter02_构筑代码质量的护城河/index.md)。

文件 `book/part1_软件工程筑基/chapter01_开发者的元技能/demo_index.py`（验证本章环境与 m2t 教学包可用）：

```{code-cell} ipython3
# 文件 book/part1_软件工程筑基/chapter01_开发者的元技能/demo_index.py
