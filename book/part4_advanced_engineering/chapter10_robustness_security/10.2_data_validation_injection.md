...
文件 `book/part4_advanced_engineering/chapter10_robustness_security/demo_sql_injection.py`：
...
> **跨平台提示**：`sqlite3` 占位符在所有平台行为一致，均为 `?`（qmark）或 `:name`（named）。路径示例 `pathlib.Path(td) / "inject.db"` 在 Windows 上自动转为 `\`，在 macOS / Linux 上为 `/`；示例中统一用 `/` 书写即可。
...
文件 `book/part4_advanced_engineering/chapter10_robustness_security/demo_xss_escape.py`：
...
文件 `book/part4_advanced_engineering/chapter10_robustness_security/demo_csrf_token.py`：
...
```bash
# 前端携带 CSRF 的请求示意（macOS / Linux / Windows 通用）
