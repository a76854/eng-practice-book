...
文件 `book/part4_advanced_engineering/chapter10_robustness_security/demo_error_boundary.py`：
...
文件 `book/part4_advanced_engineering/chapter10_robustness_security/demo_graceful_degradation.py`：
...
```bash
# 前端感知降级的响应示意（macOS / Linux / Windows 通用）
curl http://localhost:8000/api/tasks/t10 | jq .
# 正常: {"minutes":"...","degraded":false}
# 降级: {"minutes":"... [降级：来自缓存]","degraded":true,"reason":"LLM timeout"}
```

> **跨平台提示**：重试中的 `time.sleep` 在 Windows / macOS / Linux 上行为一致，单位均为秒；涉及超时配置时，建议用浮点秒（如 `0.5`、`60`）而非毫秒，避免“毫秒/秒”混淆。路径与日志见 [第10章 索引](index.md) 的跨平台约定。
