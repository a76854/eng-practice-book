...
```bash
# 按 task_id 检索日志的示意（macOS / Linux）
grep '"task_id": "t10"' app.log | jq .
# Windows (PowerShell) 等价
Select-String -Pattern '"task_id": "t10"' app.log
```

> **跨平台提示**：日志文件路径在 macOS / Linux 上常为 `/var/log/app/app.log`，在 Windows 上为 `C:\logs\app\app.log`；代码中用 `pathlib.Path("logs/app.log")` 可跨平台拼接，示例中统一用 `/` 书写。`logging` 的 `FileHandler` 在两类系统上行为一致，无需分支。
