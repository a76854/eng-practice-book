...
以 `deploy-demo/docker-compose.yml` 为例（教学最小二服务，镜像 MeetingToText 的“前端静态 + 后端动态”分离）：
...
> **跨平台提示**：Compose 文件中的端口与路径在 Windows / macOS / Linux 均一致；`healthcheck` 的探测命令在容器内执行（Linux 环境），与宿主机操作系统无关；宿主机访问时统一用 `http://localhost:8000` 与 `http://localhost`。

## 可运行示例一：用 PyYAML 解析并校验 `deploy-demo/docker-compose.yml`

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_compose_parse.py`：
...
path = _find("deploy-demo/docker-compose.yml")
assert path.exists(), f"missing docker-compose.yml (cwd={pathlib.Path.cwd()})"
...
assert backend.get("build", {}).get("dockerfile") == "Dockerfile.backend"
...
# --- backend ---
# build: {'context': '.', 'dockerfile': 'Dockerfile.backend'}
# build.dockerfile OK: Dockerfile.backend
# environment: {'MTT_DATA_DIR': '/data'}
...
```bash
# 本地校验 Compose 合法性（无需守护进程，纯 YAML 校验）
.venv/bin/python -c "import yaml, pathlib; yaml.safe_load(pathlib.Path('deploy-demo/docker-compose.yml').read_text()); print('yaml ok')"
# 若已安装 Docker，可进一步做配置校验（本章不要求守护进程，教学中可选）
# docker compose -f deploy-demo/docker-compose.yml config -q && echo "compose config ok"
```

## 可运行示例二：拓扑推演与反例——为何需要 `service_healthy`

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_compose_health.py`：
...
path = _find2("deploy-demo/docker-compose.yml")
...
> **工程启示**：Compose 的价值不在“把两个容器写在一个文件里”，而在把“谁依赖谁的就绪”显式化。`service_healthy` 把人肉的“等一等再访问”变为可校验的拓扑约束；`healthcheck` 的探测路径（`api/health`）则与 [第10章 错误边界](../chapter10_robustness_security/10.4_error_boundary_graceful_degradation.md) 的可观测约定相互印证——无健康探针的服务无法参与可靠编排。

```bash
# 查看 Compose 拓扑（macOS / Linux / Windows 通用，纯文本）
cat deploy-demo/docker-compose.yml
# 校验 YAML 合法性（本地，无需守护进程）
.venv/bin/python -c "import yaml, pathlib; print(yaml.safe_load(pathlib.Path('deploy-demo/docker-compose.yml').read_text())['services'].keys())"
```
