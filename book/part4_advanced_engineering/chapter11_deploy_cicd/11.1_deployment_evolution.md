...
> **跨平台提示**：容器与虚拟环境的路径差异需留意——虚拟环境激活在 macOS / Linux 为 `source .venv/bin/activate`，Windows 为 `.venv\Scripts\activate`；容器内的路径统一为 Linux 风格 `/app`、`/data`，这正是容器抹平平台差异的价值。

## 可运行示例一：用 Python 对照演示“进程/依赖隔离”的思想

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_isolation_levels.py`：
...
```bash
# 验证虚拟环境隔离（macOS / Linux）
.venv/bin/python -c "import sys, sysconfig; print(sys.prefix != sys.base_prefix, sysconfig.get_paths()['purelib'])"
# Windows 需用
.venv\Scripts\python.exe -c "import sys, sysconfig; print(sys.prefix != sys.base_prefix, sysconfig.get_paths()['purelib'])"
```

## 可运行示例二：为何容器能终结“在我机器上能跑”

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_reproducible_env.py`：
...
dockerfile_path = _find("deploy-demo/Dockerfile.backend")
assert dockerfile_path.exists(), f"missing Dockerfile.backend (cwd={pathlib.Path.cwd()})"
...
```bash
# 查看 Dockerfile 固化的环境（macOS / Linux / Windows 通用，纯文本读取）
cat deploy-demo/Dockerfile.backend
# Windows PowerShell 亦可用
Get-Content deploy-demo/Dockerfile.backend
```
