...
教学样例 `deploy-demo/Dockerfile.backend` 为保持“最小可运行”未显式分段，但已体现多阶段的核心思想——只拷入需要的 `m2t/` 与 `pyproject.toml`，避免把 `labs/`、`book/`、`deploy-demo/` 等无关上下文送入镜像；若需前端，可在同仓增加 `FROM node:20 AS frontend-builder` 再 `COPY --from=frontend-builder /app/dist`。
...
> **跨平台提示**：`Dockerfile.backend` 中的路径统一为 Linux 风格 `/app`、`/data`，在 Windows 宿主机上构建时同样生成 Linux 镜像；构建上下文的路径分隔符由 Docker 客户端处理，正文中的 `COPY m2t/ ./m2t/` 在 Windows / macOS / Linux 均一致。

## 可运行示例一：解析 `deploy-demo/Dockerfile.backend` 的层与缓存

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_dockerfile_parse.py`：
...
path = _find("deploy-demo/Dockerfile.backend")
assert path.exists(), f"missing Dockerfile.backend (cwd={pathlib.Path.cwd()})"
...
# 预期输出:
# === Dockerfile 指令序列（层视角） ===
#   L03 FROM       python:3.12-slim
#   L04 WORKDIR    /app
#   L07 RUN        apt-get update ...
#   L12 COPY       pyproject.toml README.md ./
#   L13 COPY       m2t/ ./m2t/
#   L16 RUN        pip install --no-cache-dir -e .
#   L18 ENV        MTT_DATA_DIR=/data
#   L21 EXPOSE     8000
#   L24 CMD        ["python", "-m", "m2t.cli", "serve", ...]
# 层统计: {'CMD': 1, 'COPY': 2, 'ENV': 1, 'EXPOSE': 1, 'FROM': 1, 'RUN': 2, 'WORKDIR': 1}
# COPY 指令行号: [12, 13]
# RUN pip install 行号: [16]
# COPY 顺序 OK：pyproject.toml 在 m2t/ 之前（依赖层可长期缓存）
# 最大 COPY 行 13 < 最小 pip install 行 16 ? True
# 体积控制 OK：...
# 解析结论：...
```

```bash
# 本地查看 Dockerfile 层（macOS / Linux / Windows 通用，纯文本）
cat deploy-demo/Dockerfile.backend
# 若已安装 Docker，仅查看解析后的配置（本章不要求守护进程，教学中可选）
# docker build -f deploy-demo/Dockerfile.backend --dry-run 2>&1 | head -n 20  # 仅示意，实际构建需守护进程
```

## 可运行示例二：为何 COPY 顺序决定构建速度

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_layer_cache.py`：
...
print("=== 好顺序：先清单后代码（deploy-demo 采用） ===")
...
> **工程启示**：Dockerfile 不是脚本的堆砌，而是对“变更频率”的显式排序。把最稳定的放最前、最易变的放最后，才能让缓存命中率最大化；多阶段则把“构建时工具”与“运行时依赖”解耦，二者共同决定镜像的构建速度与体积。与 [第1章 工程化项目结构](../../part1_software_engineering/chapter01_dev_meta_skills/1.1_engineering_project_structure.md) 的“可复现依赖”相互印证——Dockerfile 把 `pyproject.toml` 的可复现性延伸到系统库与文件布局。
