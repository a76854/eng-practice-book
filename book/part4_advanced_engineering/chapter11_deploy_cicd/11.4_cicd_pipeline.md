...
触发事件的选择：`push` 保证“每次推送都验证”，`pull_request` 保证“合入前在目标分支上下文中再验证一次”，二者叠加可覆盖“分支推送”与“合入门禁”两类场景。教学样例 `deploy-demo/ci.yml` 正是此配置。

## 教学样例 `deploy-demo/ci.yml` 的门禁链路
...
> **跨平台提示**：流水线中的 `run` 默认在 `ubuntu-latest` 的 Linux 执行器上，路径为 Linux 风格 `/home/runner/work/...`；本地开发在 Windows / macOS / Linux 均用 `.venv` 复现同一套 `ruff` / `mypy` / `pytest` 命令，保证“本地绿、CI 亦绿”。

## 可运行示例一：用 PyYAML 解析并校验 `deploy-demo/ci.yml`

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_ci_parse.py`：
...
path = _find("deploy-demo/ci.yml")
assert path.exists(), f"missing ci.yml (cwd={pathlib.Path.cwd()})"
...
```bash
# 本地校验 CI YAML 合法性（无需 GitHub，纯 YAML 解析）
.venv/bin/python -c "import yaml, pathlib; print(yaml.safe_load(pathlib.Path('deploy-demo/ci.yml').read_text())['jobs']['verify']['runs-on'])"
# 本地复刻流水线的四道门禁（macOS / Linux）
...
.venv/bin/python -c "import yaml, pathlib; yaml.safe_load(pathlib.Path('deploy-demo/docker-compose.yml').read_text()); print('compose yaml ok')"
# Windows 需把 .venv/bin/python 换为 .venv\Scripts\python.exe
```

## 可运行示例二：为何门禁要分层、按序执行

文件 `book/part4_advanced_engineering/chapter11_deploy_cicd/demo_pipeline_order.py`：

```{code-cell} ipython3
# 文件 book/part4_advanced_engineering/chapter11_deploy_cicd/demo_pipeline_order.py
import pathlib, yaml

def _find2(rel: str) -> pathlib.Path:
    cand = pathlib.Path(rel)
    if cand.exists():
        return cand
    for base in [pathlib.Path.cwd(), *pathlib.Path.cwd().parents]:
...
# 1) 解析真实流水线的步骤顺序
path = _find2("deploy-demo/ci.yml")
...
print("  Windows 需把 .venv/bin/python 换为 .venv\\Scripts\\python.exe")
...
```bash
# 查看 CI 流水线声明（macOS / Linux / Windows 通用，纯文本）
cat deploy-demo/ci.yml
# 本地等价复刻（macOS / Linux）
...
# Windows 需用
.venv\Scripts\python.exe -m ruff check .; .venv\Scripts\python.exe -m mypy m2t --ignore-missing-imports; .venv\Scripts\python.exe -m pytest -q
```
