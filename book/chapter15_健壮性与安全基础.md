...
### 输入校验：扩展名、魔数、Content-Length 与空文件
...
| 魔数 | `header.startswith(b"RIFF")` 等容器签名 | `400 文件内容与音频格式不符` | 防止“改后缀的伪文件”过检（见"去掉魔数校验"实验） |
...
### 限流：固定窗口、Retry-After 与单进程 caveat
