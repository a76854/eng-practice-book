...
文件 `book/part4_现代工程进阶与交付/chapter11_部署容器化与持续集成/demo_summary.py`（本章贯通校验：用文本解析串联“演进思想 → Dockerfile → Compose → CI”最小闭环）：

```{code-cell} ipython3
# 文件 book/part4_现代工程进阶与交付/chapter11_部署容器化与持续集成/demo_summary.py
