...
文件 `book/part4_现代工程进阶与交付/chapter11_部署容器化与持续集成/demo_isolation_levels.py`：
...
文件 `book/part4_现代工程进阶与交付/chapter11_部署容器化与持续集成/demo_reproducible_env.py`：
...
> **工程启示**：部署演进的本质是把“隐式约定”变为“显式、可版本控制、可哈希校验”的构建输入。虚拟环境让 Python 依赖可复现，容器则把系统库、文件布局与启动命令一并纳入版本控制；二者叠加，才得到“在任何机器上可复现”的交付物。与 [第1章 依赖与虚拟环境](../../part1_软件工程筑基/chapter01_开发者的元技能/1.2_依赖与虚拟环境.md) 的隔离原理相互印证——容器是虚拟环境思想在操作系统层的延伸。
