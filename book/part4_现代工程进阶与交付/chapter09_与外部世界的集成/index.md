...
- [9.1 第三方服务集成模式](9.1_第三方服务集成模式.md) —— SDK vs HTTP 客户端：何时包一层、何时直接调；超时、重试、熔断与错误脱敏
- [9.2 语音识别接入](9.2_语音识别接入.md) —— 云端签名的本质、音频格式归一（声道/采样率/编码）与多形状结果归一
- [9.3 大模型接口设计](9.3_大模型接口设计.md) —— 结构化输出（JSON 模式）、流式 SSE 的增量解析与首字时延
- [9.4 异步任务队列 Celery](9.4_异步任务队列Celery.md) —— 为何长时任务要异步化；broker / worker / result 的最小队列抽象（不依赖 Celery）
...
> **跨平台约定**：本章所有涉及路径与环境激活的命令均标注 Windows / macOS / Linux 差异，详见各小节对照表；正文跨章引用一律使用相对链接，如 [第1章 开发者的元技能](../../part1_软件工程筑基/chapter01_开发者的元技能/index.md) 与 [第6章 并发模型与性能工程](../../part2_后端开发全景与核心基石/chapter06_并发模型与性能工程/index.md)。

文件 `book/part4_现代工程进阶与交付/chapter09_与外部世界的集成/demo_index.py`（验证本章环境与 m2t 教学包可用）：

```{code-cell} ipython3
# 文件 book/part4_现代工程进阶与交付/chapter09_与外部世界的集成/demo_index.py
import sys, pathlib
