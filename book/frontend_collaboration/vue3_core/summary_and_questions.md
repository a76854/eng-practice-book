...
2. **请分析**：模板里写 `{{ result.title }}`，和写 `document.createElement("a").textContent = r.title`，一个是声明式、一个是命令式。结合搜索页"加载中/出错/有结果"三种状态的切换，讨论声明式模板省掉了哪些手工活，又带来了哪些排查上的代价。
3. **请判断**：一个页面同时有"搜索框"和"结果列表"两个区域，都要用到关键词和搜索结果。你何时用 Props 往下传、何时提升到 Pinia？给出判断标准，并说明"提得太早"和"提得太晚"各会付出什么代价。
...
8. **请设计**：前端状态是缓存、后端是真相。为"收藏文档"设计一套"前端先展示、后台再确认"的交互，并说明网络失败时前端该如何退回、如何提示，才能既响应快又不误导用户。
