...
- 本书为什么要以一个具体框架为载体来讲前端？Vue 3 能做什么？
- Vue 3 的核心特性（单文件组件、声明式渲染、响应式、组件化）分别解决什么问题？
- 什么是组合式 API？它和选项式 API 在组织代码上有什么区别？
- 为什么前端需要 Vite？它在开发期与生产期各承担什么角色？

上一节对比了三大框架的取舍。接下来不再争论谁更好，而是确定以一个具体框架为载体，把框架到底怎么用这件事讲清楚：本书选择 Vue 3，配套的构建工具是 Vite。

> 学开车的人不需要先论证哪款车最好，只需要挑一辆具体、好上手的车，把驾驶这件事学会。同理，学前端不靠比较框架，而靠挑一个具体框架把它用熟。本书挑 Vue 3 这辆上手快、结构透明的车，把前端开发这件事学会。

本节不争论选型，只做一件事：把 Vue 3 这辆车介绍清楚：它有哪些能力、怎么组织代码、需要什么工具才能跑起来。
...
| 特性 | 解决什么 | 在上例中的体现 |
| --- | --- | --- |
| 单文件组件（SFC） | 一个组件一个文件，模板、逻辑、样式聚在一处 | `<template>` 与 `<script>` 同在一个 `.vue` 文件 |
