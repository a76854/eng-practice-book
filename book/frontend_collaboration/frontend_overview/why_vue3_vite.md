...
> 去罗马的路有很多，现在你要做的事不是挑选一条近的路，先走吧。走着走着你就这道那条路更适合你
...
| 特性 | 解决什么 | 在上例中的体现 |
| --- | --- | --- |
| 单文件组件（SFC） | 一个组件一个文件，模板、逻辑、样式聚在一处 | `<template>` 与 `<script>` 同在一个 `.vue` 文件^[当然，vue3的开发者不傻，肯定能拆分，分别定义，然后import啊。这里就是一个示例] |
