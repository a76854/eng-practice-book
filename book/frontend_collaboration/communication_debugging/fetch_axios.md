...
上一节画清了整条链路，本节看链路的第一步：前端怎么把请求发出去。本节在整章的位置是"发"，只回答"请求怎么离境"，不碰等待、契约、跨域这些下游的事。目的是建立一条选型依据：两个工具都能把请求发出去，差别不在"能不能"，而在"每次都要带的那些重复劳动由谁来收敛"。

## `fetch`

浏览器里的 JS 不能直连网络：所有 HTTP 请求都要经浏览器之手发出。`fetch` 就是浏览器留的这道原生门，调用它等于告诉浏览器"替我发一次请求，结果回来叫我"，返回的 Promise 就是"叫我"的方式。先记住这道门的位置，它的形状就顺理成章了。
...
`fetch` 这副"半成品"的样子，三件事都有来由：

1. **只管发，不管对错**：对浏览器来说，404、500 也是"成功送达"的响应，丢没丢包它分得清，对错是业务的事，所以 `res.ok` 留给你判。网络不通才抛错，因为那是"没送到"。
2. **响应体默认是流**：结果可能很大，浏览器不想替你一次性全读进内存，`res.json()` 这一调等于你说"我要全读出来转成 JSON"，主动权在你。
3. **超时、重试、默认头**统统没有：等多久算太久、失败要不要再试，这些是各家业务的私事，浏览器不替你做主，要自己包一层（下一小节就包）。

正因为"只开门、不包办"，`fetch` 最轻、零依赖；请求量小、没有登录态时，它就是最短的那条路。

## `axios`

`axios` 是一个请求库，作用只有一句话：把每个请求都要重复写的那几件事收敛到一处。它不发明新能力，只是把上一小节自包的那层（超时、判状态、默认头），再加上"跨请求的公共逻辑"（token、日志、统一报错），做成开箱即用的样子：
...
## `fetch` 缺的三件，自己包一层长什么样

`fetch` 缺超时、缺"非 2xx 抛错"、缺默认头，自己包一层大约十来行：

```javascript
// 自包一层：超时 + 判状态 + 转 JSON，收敛三处重复
async function apiGet(path, params = {}, timeoutMs = 8000) {
  const ctrl = new AbortController()
  const timer = setTimeout(() => ctrl.abort(), timeoutMs)
  try {
    const qs = new URLSearchParams(params).toString()
    const res = await fetch(`${path}?${qs}`, { signal: ctrl.signal })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
...
这个十来行的函数，就是 `axios` 替你写掉的部分。请求量小、没有登录态时，手写这一层等于零依赖实现"够用"；一旦每个请求都要带 token、每个失败都要统一提示，这一层会膨胀成新的"自研 axios"，不如直接用 `axios` 的拦截器。
...
- `fetch` 缺的三件可自包一层补上；公共逻辑一多，自研层会膨胀成新的 axios，不如直接选型。
