...
CD 在 CI 通过后自动把代码部署到目标环境（测试环境、预发布环境、生产环境）。它是 CI 的下一站，但不属于"测试与质量"范畴。基本原则是：**CI 没通过，CD 不触发**。这部分我们将在[CI/CD 流水线](../../part4_advanced_engineering/chapter11_deploy_cicd/11.4_cicd_pipeline.md)处讲解。