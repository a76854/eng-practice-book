...
- **工程化项目结构是协作的起点**：`src` 布局通过物理隔离暴露导入与打包问题，`pyproject.toml` 以 PEP 517/518/621 统一构建与元数据，二者共同构成“可安装、可测试”的最小工程契约。MeetingToText 的教学包 `m2t` 与真实项目 `backend/app` 分层即是该思想的两种落地。
...
- **Shell 的核心是可组合的文本流水线**：文件系统提供统一的路径抽象，进程提供隔离的执行上下文，管道提供“单件工具 + 文本流”的组合能力；`pathlib` / `subprocess` / 生成器表达式可在 Python 中跨平台复刻这些思想。
...
2. **隔离边界**：虚拟环境隔离了 `site-packages`，但未隔离系统库与环境变量。结合 `MTT_DATA_DIR` 与 `MODELSCOPE_CACHE` 的用法，讨论“环境隔离”与“配置外置”的边界应如何划分。
3. **管道与函数**：Shell 管道 `find | xargs | sort` 与 Python 的生成器管道各有何优劣？在 MeetingToText 的“上传 → 切片 → 转写 → 纪要”流水线中，哪一段更适合用管道思维，哪一段更适合用函数调用？
4. **自动化粒度**：`Makefile` 与 Python 脚本在自动化中的分工应如何界定？当一个自动化任务需要跨 Windows / macOS / Linux 时，你会如何决定用 `Makefile`、`Shell` 还是 `Python` 实现？
5. **Git 工作流演进**：假设 MeetingToText 从单人课程项目演进为 10 人团队的持续交付产品，你会如何从 GitHub Flow 逐步引入 `release` 分支或 feature flag？分支策略的变更会对 CI 流水线与发布流程带来哪些连锁影响？
