...
**全局 Python 环境就像城市里的共享单车**——谁都能骑，但不能保证上车时座椅高度和你想要的一样。你的项目需要 `numpy==1.26`，隔壁项目需要 `numpy==2.0`，但只有一个位置，你们轮流拧螺丝、换轮胎，最终谁也骑不舒服。虚拟环境是**私家车库**——每个项目拥有一辆完全属于它的车，零件、油量、胎压，全部独立调校。

## 从 `pyproject.toml` 到可运行环境

[工程化项目结构](./1.1_engineering_project_structure.md)我们写好了 `pyproject.toml`——这份文件定义了"项目是什么、依赖谁、怎么安装"。但它目前只是一份**声明**，就像一张写满食材的菜谱，还没真正下锅。
