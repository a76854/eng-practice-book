...
- [1.5 Git 工作流——用分支与 PR 协作](1.5_git_workflow.md) —— 用分支与 PR 完成协作

本章所有示例均可在书仓根目录的 `.venv` 环境中复现。正文示例优先使用通用项目名 `myproject` / `mypackage` / `demo`，通过教学包 `m2t`（见 [m2t 源码](../../..//m2t/audio.py)）提供可运行的最小实现，无需启动任何外部服务即可验证概念。
