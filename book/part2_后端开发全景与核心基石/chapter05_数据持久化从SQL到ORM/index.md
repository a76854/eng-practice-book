...
- [5.1 关系型数据库原理](5.1_关系型数据库原理.md) —— 事务 ACID、隔离级别、索引与锁概览：数据为什么需要“契约”
- [5.2 SQL 进阶](5.2_SQL进阶.md) —— JOIN、子查询、窗口函数：从“查得到”到“查得巧”
- [5.3 Python 操作数据库三境界](5.3_Python操作数据库三境界.md) —— 原生 `sqlite3` / 驱动层 `asyncpg` / ORM 层 `SQLAlchemy 2.0` 的分层与选型
- [5.4 数据库迁移 Alembic](5.4_数据库迁移Alembic.md) —— 版本控制与回滚思想：用 SQLite 模拟 Alembic 的迁移步骤
- [5.5 数据建模原则](5.5_数据建模原则.md) —— 范式化与反范式化：何时拆表、何时冗余

此外，本章所有可执行示例均可在书仓 `.venv` 环境中复现；涉及 MeetingToText 的片段复用 `m2t.store.TaskStore`（见 [m2t 源码](../../../m2t/store.py) 的精简实现），无需启动真实的 ASR 或 LLM 服务。

> **跨平台约定**：本章所有涉及路径与环境激活的命令均标注 Windows / macOS / Linux 差异，详见各小节对照表；正文跨章引用一律使用相对链接，如 [第1章 开发者的元技能](../../part1_软件工程筑基/chapter01_开发者的元技能/index.md) 与 [第4章 HTTP 与 RESTful 架构](../chapter04_HTTP与RESTful架构/index.md)。

文件 `book/part2_后端开发全景与核心基石/chapter05_数据持久化从SQL到ORM/demo_index.py`（验证本章环境与 `m2t.store.TaskStore` 可用）：

```{code-cell} ipython3
# 文件 book/part2_后端开发全景与核心基石/chapter05_数据持久化从SQL到ORM/demo_index.py
