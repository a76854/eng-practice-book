...
本章与第二篇前三章形成闭环：[第3章 后端职责与分层](../chapter03_后端开发到底是什么/index.md) 给出“请求如何穿过系统”，[第4章 HTTP 语义](../chapter04_HTTP与RESTful架构/index.md) 约束“如何暴露契约”，[第5章 持久化](../chapter05_数据持久化从SQL到ORM/index.md) 解决“如何存得可靠”，本章则回答“如何跑得并发且快得可证明”——四章共同构成后端核心基石。
...
5. **数据库并发与性能的权衡**：`m2t/store.py` 的 `idx_tasks_status` 能加速按状态查询，但会拖慢写入。若任务表的读写比从 10:1 变为 1:10，你是否仍保留该索引？请结合 [第5章 索引代价](../chapter05_数据持久化从SQL到ORM/5.1_关系型数据库原理.md) 说明判断依据，并思考如何用 `EXPLAIN QUERY PLAN` 验证。
