...
- [4.1 HTTP 协议精髓](4.1_HTTP协议精髓.md) —— 方法、幂等性、状态码语义与 Header 设计：HTTP 为什么这样设计
- [4.2 RESTful 成熟度模型](4.2_RESTful成熟度模型.md) —— 从 RPC-style 到 Hypermedia：Richardson 四级的演进与选型
- [4.3 FastAPI 路由设计](4.3_FastAPI路由设计.md) —— 路径/查询/请求体参数的声明式校验与依赖注入
- [4.4 异常处理与全局响应](4.4_异常处理与全局响应.md) —— 统一返回格式与全局异常处理的协作价值
- [4.5 OpenAPI 契约测试](4.5_OpenAPI契约测试.md) —— 自动生成 Swagger 与前后端契约测试
...
> **跨平台约定**：本章所有涉及路径与环境激活的命令均标注 Windows / macOS / Linux 差异，详见各小节对照表；正文跨章引用一律使用相对链接，如 [第1章 开发者的元技能](../../part1_软件工程筑基/chapter01_开发者的元技能/index.md) 与 [第3章 后端开发到底是什么](../chapter03_后端开发到底是什么/index.md)。

文件 `book/part2_后端开发全景与核心基石/chapter04_HTTP与RESTful架构/demo_index.py`（验证本章环境与 FastAPI 核心依赖可用）：

```{code-cell} ipython3
# 文件 book/part2_后端开发全景与核心基石/chapter04_HTTP与RESTful架构/demo_index.py
import sys, pathlib
