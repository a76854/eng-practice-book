...
- [3.1 职责边界](3.1_职责边界.md) —— 后端的四条职责线：API 设计、业务逻辑、数据流转、安全防护，以及一条请求如何穿过它们
- [3.2 语言横向对比](3.2_语言横向对比.md) —— Java / C# / Go / PHP 与 Python 的客观对比：适用场景、生态与代价，而非“谁更好”
- [3.3 框架生态谱系](3.3_框架生态谱系.md) —— 从全栈 Django 到微架构 Flask 再到高性能 FastAPI，谱系如何形成
- [3.4 为何选择 FastAPI](3.4_为何选择FastAPI.md) —— 异步原生、自动文档、类型安全：FastAPI + Pydantic 的组合为什么适合本课程
- [3.5 分层架构落地](3.5_分层架构落地.md) —— Controller-Service-Repository 在 Python 中的一种朴素落地，让每一层可独立测试与替换

此外，本章所有示例均可在书仓 `.venv` 环境中复现；涉及 MeetingToText 的片段统一通过教学包 `m2t` 复用（如 `m2t/store.py` 的 `TaskStore`），无需启动真实服务。

> **跨平台约定**：本章所有涉及路径与环境激活的命令均标注 Windows / macOS / Linux 差异，详见各小节对照表；正文跨章引用一律使用相对链接，如 [第1章 开发者的元技能](../../part1_软件工程筑基/chapter01_开发者的元技能/index.md) 与 [第2章 构筑代码质量的护城河](../../part1_软件工程筑基/chapter02_构筑代码质量的护城河/index.md)。

文件 `book/part2_后端开发全景与核心基石/chapter03_后端开发到底是什么/demo_index.py`（验证本章环境与核心依赖可用）：

```{code-cell} ipython3
# 文件 book/part2_后端开发全景与核心基石/chapter03_后端开发到底是什么/demo_index.py
import sys, pathlib
