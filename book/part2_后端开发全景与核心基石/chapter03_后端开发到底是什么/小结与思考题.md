...
- **贯穿启示**：本章的“职责—选型—框架—分层”四步为后续章节提供了锚点—— [第4章的 HTTP 与 RESTful](../chapter04_HTTP与RESTful架构/index.md) 细化契约，[第5章的持久化](../chapter05_数据持久化从SQL到ORM/index.md) 深化 Repository，[第6章的并发](../chapter06_并发模型与性能工程/index.md) 展开异步，[第10章的安全](../../part4_现代工程进阶与交付/chapter10_健壮性与安全底线/index.md) 回到纵深防护。
...
4. **异步的边界**：FastAPI 的异步能提升 I/O 密集场景的吞吐，但在 CPU 密集的音频预处理中为何未必有效？结合 [第6章的 GIL 与异步](../../part2_后端开发全景与核心基石/chapter06_并发模型与性能工程/index.md) 讨论何时该用异步、何时该用进程或外部服务。
...
文件 `book/part2_后端开发全景与核心基石/chapter03_后端开发到底是什么/demo_summary.py`（本章贯通校验：分层 + 持久化 + 契约的最小闭环）：

```{code-cell} ipython3
# 文件 book/part2_后端开发全景与核心基石/chapter03_后端开发到底是什么/demo_summary.py
