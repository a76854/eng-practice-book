...
> **跨平台提示**：URL 拼接与路径处理与操作系统无关；若示例涉及本地文件路径，则用 `pathlib.Path` 兼容 Windows（`\`）与 macOS / Linux（`/`），虚拟环境激活为 `source .venv/bin/activate`（macOS / Linux）与 `.venv\Scripts\activate`（Windows）。
...
```javascript
// 文件 book/samples/vue-min/templates_tasks.html（Jinja2，后端渲染示意）
...
前后端分离（Vue 风格，仅示意，不执行，对应 book/samples/vue-min/src/App.vue）：

```javascript
// 文件 book/samples/vue-min/src/App.vue（节选，前后端分离示意）
...
文件 `book/part3_frontend_collaboration/chapter07_frontend_overview/demo_render_compare.py`（用 dataclass 模拟两种形态：后端模板在服务端拼接 HTML，前后端分离在服务端返回 JSON、客户端拼接，本地可复现，无网络）：

```{code-cell} ipython3
# 文件 book/part3_frontend_collaboration/chapter07_frontend_overview/demo_render_compare.py
...
> **与后续章节的衔接**：本节的“接口契约”将在 [第4章 HTTP 与 RESTful](../../part2_backend_development/chapter04_http_restful/index.md) 的 OpenAPI 与 [第8章 路由与状态](../../part3_frontend_collaboration/chapter08_vue3_core/index.md) 的前端路由中进一步展开；MeetingToText 的任务列表页即采用“前端路由 + `/api/tasks` 接口”的分离形态，见 [vue-min 样例](../../samples/vue-min/src/App.vue)。

```bash
# 本地验证 URL 拼接与 JSON 产物的确定性（macOS / Linux）
.venv/bin/python -c "from urllib.parse import urljoin; print(urljoin('http://localhost:8000','/api/tasks'))"
# Windows 需用
.venv\Scripts\python.exe -c "from urllib.parse import urljoin; print(urljoin('http://localhost:8000','/api/tasks'))"
```
