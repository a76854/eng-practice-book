...
文件 `book/part3_frontend_collaboration/chapter08_vue3_core/demo_component.py`（用 Python 类模拟 Props/Emits/Slots 三条链路，本地可复现，无网络）：

```{code-cell} ipython3
# 文件 book/part3_frontend_collaboration/chapter08_vue3_core/demo_component.py
...
> **与全书的衔接**：本节的组件边界划分是 [8.3 路由](8.3_routing_management.md) 中“路由级组件 vs 子组件”的前提，也是 [8.4 Pinia](8.4_cross_component_state_pinia.md) 中“何时用 Props 逐层传、何时提升到 Store”的判断依据；组件的 `onMounted` 数据拉取将在路由守卫中被更精细地控制。
