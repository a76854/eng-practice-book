...
> **跨平台提示**：组件化与操作系统无关；本地验证时用 `pathlib.Path` 处理 `src/components/` 等路径，虚拟环境激活为 `source .venv/bin/activate`（macOS / Linux）与 `.venv\Scripts\activate`（Windows）。
...
# Windows 需用
.venv\Scripts\python.exe -c "import pathlib; print(pathlib.Path('src/components').exists())"
```
